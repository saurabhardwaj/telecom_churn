...
import {
  ...
  processPDF,
} from "../../api/service/investment-details.service";
...
export default function MyInvestmentMemos() {
  ...
  const getData = async () => {
    const history = [];
    const data = await getAllCompletedInvestment();
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: any, record: any) => (
        <a onClick={() => downloadPdf(record)}>Download</a>
      ),
    },
  ];
              ...
              <div className="bg-[#FFFFFF] w-[90%] h-fit min-h-full relative">
                <div className="w-full min-h-[90%] h-full pl-[47px] pr-[40px] py-[8px]">
                  ...
                  <div className=" w-full h-full mt-11 px-4 py-4">
                    <div className="text-[#000000] text-lg">Total Downloads : <span>{dataSource.length}</span></div>
